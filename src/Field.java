...
public class Field implements Serializable{
    ...
    public Field(int width, int height){
        this.width = width;
        this.height = height;
        fieldCell = new Landscape[height][width];
    }
    ...
    public void setRandomLandscape(){
        ...
        for (int j = 0; j<height; j++){
            for (int i = 0; i<width; i++){
                ...
                if (randomNumber == 1)
                    fieldCell[j][i] = new Cobblestoun();
                else if (randomNumber == 2)
                    fieldCell[j][i] = new Grass();
                else if (randomNumber == 3)
                    fieldCell[j][i] = new Gravel();
                else if (randomNumber == 4)
                    fieldCell[j][i] = new Sand();
                else if (randomNumber == 5)
                    fieldCell[j][i] = new Snow();
            }
        }
    }
    ...
    public void print(){
        for (int j = 0; j<height; j++){
            for (int i = 0; i<width; i++){
                System.out.print(fieldCell[j][i].name + " ");
            }
            System.out.println("\n");
        }
    }
        ...
        while (currentCell.getX() != finishCell.getX() || currentCell.getY() != finishCell.getY()){
            ...
            if (currentCell.getX()-1 >= 0 && !fieldCell[currentCell.getY()][currentCell.getX()-1].isVisited){
                ...
                if (neighborCell != null){
                    if (neighborCell.getDistance() > currentCell.getDistance() + fieldCell[currentCell.getY()][currentCell.getX()].time){
                        neighborCell.setDistance(currentCell.getDistance() + fieldCell[currentCell.getY()][currentCell.getX()].time);
    ...
    public Field load() throws IOException, ClassNotFoundException {
        ...
        Landscape[][] fieldCell = new Landscape[height][width];
        for (int j=0; j<width; j++)
            for (int i=0; i<height; i++)
                fieldCell[j][i] = (Landscape) inputStream.readObject();
    ...
    public void  findAllPath(Cell finishCell, ArrayList<Cell> currentPath, ArrayList<ArrayList<Cell>> allPaths, int minimalPathLength, int currentPathLength){
        ...
        if (currentPathLength == minimalPathLength && currentCell.getX() == finishCell.getX() && currentCell.getY() == finishCell.getY()){
            ...
            if (currentCell.getX()-1 >= 0 && (currentPath.size() == 1 || currentPath.get(currentPath.size()-2).getX() != currentCell.getX()-1)){
                ...
                currentPathLength += fieldCell[currentCell.getY()][currentCell.getX()].time;
